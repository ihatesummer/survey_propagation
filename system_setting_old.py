...
def main(n_user, n_resource, ap_positions,
    ...
    G_anr = get_G(np.size(ap_positions, axis=0),
                  n_user, n_resource)
    y = get_y(x, ap2user_distances, n_resource, G_anr)
    if save_path=="debug":
...
def generate_user_positions(n_user, ap_positions, max_distance):
    ...
    while n_sample != n_user:
        ...
        # non-symmetric purely random sampling
        # is_covered = False
...
def get_y(x, ap2user_distances, n_resource, G_anr):
    ...
    for i in range(len(x)):
        if type(x[i]) == int:
            for r in range(n_resource):
                sum_term = 0
                for ap in range(n_ap):
                    d_cube = ap2user_distances[ap, i]**3
                    sum_term += G_anr[ap, i, r] / d_cube
            ...
            for r in range(n_resource):
                ...
                for ap_pair in ap_pair_list:
                    ap1, ap2 = ap_pair
                    snir1 = np.log2(
                        1 + (G_anr[ap1, i1, r]/ap2user_distances[ap1, i1]**3)
                            / (G_anr[ap2, i1, r]/ap2user_distances[ap2, i1]**3 + (STD_HAT**2)))
                    snir2 = np.log2(
                        1 + (G_anr[ap2, i2, r]/ap2user_distances[ap2, i2]**3)
                            / (G_anr[ap1, i2, r]/ap2user_distances[ap1, i2]**3 + (STD_HAT**2)))
...
if __name__=="__main__":
    main(n_user=15, n_resource=3,
         ap_positions=np.array([[0, 0], [10, 0]]),
         max_distance=10, seed=0,
         save_path="debug")
